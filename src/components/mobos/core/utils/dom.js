(function (window, document, mobos) {
  ...
  function domReady() {
    ...
    for (var x = 0; x < readyCallbacks.length; x++) {
      mobos.requestAnimationFrame(readyCallbacks[x]);
    }
    readyCallbacks = [];
  ...
  mobos.DomUtil = {
    ...
    /**
     * @ngdoc method
     * @name mobos.DomUtil#requestAnimationFrame
     * @alias mobos.requestAnimationFrame
    ...
    /**
     * @ngdoc method
     * @name mobos.DomUtil#animationFrameThrottle
     * @alias mobos.animationFrameThrottle
     * @description
    ...
    ready: function (cb) {
      if (isDomReady) {
        mobos.requestAnimationFrame(cb);
      } else {
    ...
    getComputedBounds: function (node) {
      var props = document.defaultView.getComputedStyle(node, null)
      return {
        top: parseInt(props.getPropertyValue('top')),
        bottom: parseInt(props.getPropertyValue('bottom')),
        height: parseInt(props.getPropertyValue('height')),
        width: parseInt(props.getPropertyValue('width'))
      }
    },

    getElemPositionInParent: function(child, parent) {
  ...
  //Shortcuts
  mobos.requestAnimationFrame = mobos.DomUtil.requestAnimationFrame;
  mobos.cancelAnimationFrame = mobos.DomUtil.cancelAnimationFrame;
  mobos.animationFrameThrottle = mobos.DomUtil.animationFrameThrottle;
