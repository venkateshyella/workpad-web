...
(function() {
  ...
  function Util($cacheFactory, $document, $timeout, $q, $window, $Constant) {
    var Util;
    ...
    return Util = {
      ...
      transitionEndPromise: function(element, opts) {
        ...
        element.on($Constant.CSS.TRANSITIONEND, finished);
        function finished(ev) {
          // Make sure this transitionend didn't bubble up from a child
          if (!ev || ev.target === element[0]) {
            element.off($Constant.CSS.TRANSITIONEND, finished);
            deferred.resolve();
          }
        }
  ...
  Util.$inject = ['$cacheFactory', '$document', '$timeout', '$q', '$window', '$Constant'];
