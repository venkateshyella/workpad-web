...
(function () {
	...
	angular.module('app')
		.service('TaskVaultAdmin', [
			'$q', 'mDialog', 'blockUI',
			'Connect', 'URL', 'Lang',
			'DataProvider', 'VaultServices', 'FileSystem', 'FileService', 'VaultFilesWorkflow', 'VaultFileRename',
			TaskVaultAdminService
		]);

	function TaskVaultAdminService($q, Dialog, blockUI, Connect, URL, Lang, DataProvider, VaultServices, FileSystem, FileService, VaultFilesWorkflow, VaultFileRename) {

		var LANG = Lang.en.data;
		return {
			getTaskFilesList: getTaskFilesList,
		...
		function getTaskFilesList(taskId,params) {
			blockUI.start("Loading Task Data ", {
				status: 'isLoading'
			});
			return VaultServices.FetchTaskVaultFiles(taskId, params)
		...
		function findAndCopyVaultFiles(params) {
			...
			function fetchTaskCopyDestination() {
				return _fetchCopyDestinationList(params.catId);
			}
		}

		function _fetchCopyDestinationList(taskId) {
			var taskModel = DataProvider.resource.Task.get(taskId);
			return taskModel.fetchVaultFileCopyDestinations()
				.then(function (res) {
		...
		function fetchVaultFilesWithAdminRights(taskId) {

			return DataProvider.resource.File.findAll({
					taskId: taskId
				}, {
					url: URL.TASK_VAULT_OWNER_FILE_LIST,
					bypassCache: true
				})
				.then(function (files) {
					return files;
		...
		function fetchVaultFoldersWithAdminRights(taskId) {

			return DataProvider.resource.File.fetchOwnedFolderList(taskId)
				.then(function (folders) {
					return folders;
				});
		}

		function findAndRemoveVaultFiles(taskId) {
			var fileList = [];
			var deferred = $q.defer();
			blockUI.start(LANG.ORGANISATION.LOADING_MSG.FETCHING_OWNER_FILES);
			fetchVaultFilesWithAdminRights(taskId)
				.then(function (files) {
					fileList = files;
					if (files && files.length > 0) {
						...
						VaultFilesWorkflow.selectAndExecuteActionOnFiles(fileList,
							function (fileList) {
								...
								var taskModel = DataProvider.resource.Task.get(taskId);
								taskModel.deleteFiles(fileList)
									.then(function (res) {
										deferred.resolve(res);
					...
					if (files && files.length > 0) {
						...
						VaultFileRename.renameFiles(files,
							function (updatedFiles) {
								...
								var taskModel = DataProvider.resource.Task.get(params.taskId);
								taskModel.updateFiles(DataProvider.pluckAttrArrayProperties(updatedFiles, ['id', 'fileDisplayName']), {
