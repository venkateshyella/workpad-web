...
blkUI.directive('blockUiContainer', ["blockUIConfig", "blockUiContainerLinkFn", function (blockUIConfig, blockUiContainerLinkFn) {
  ...
  return function ($scope, $element, $attrs) {
    ...
    // Private function to stop the blocker from the scope.
...
blkUI.factory('blockUI', ["blockUIConfig", "$timeout", "blockUIUtils", "$document", function(blockUIConfig, $timeout, blockUIUtils, $document) {
  ...
  function BlockUI(id) {
    ...
    self._waitforDismissTimer = null;

    this.start = function(message, options) {
      ...
      state.options = angular.copy(blockUIConfig.options);
      ...
      if (!startPromise) {
        startPromise = $timeout(function() {
          startPromise = null;
          state.blocking = true;
          state._startTimestamp = new Date();
        }, blockUIConfig.delay);
      }
    };
    ...
    this.stop = function(message, options) {
      options = options || {};
      angular.extend(state.options, options);
      var stopTime = new Date();
      var waitForDismiss =
        state._startTimestamp.getTime() - stopTime.getTime()
        - blockUIConfig.wait;
      waitForDismiss = waitForDismiss > 0 ? waitForDismiss : 0;

      if(angular.isDefined(message)) {
        state.message = message;
        waitForDismiss = waitForDismiss + state.options.statusMessageShowWait;
      }

      //console.log(waitForDismiss);
      this._waitforDismissTimer = $timeout(function() {
        "use strict";
        state.blockCount = Math.max(0, --state.blockCount);

        if (state.blockCount === 0) {
          $timeout.cancel(self._waitforDismissTimer);
          self.reset(true);
        }
      }, waitForDismiss);
    };
    ...
    this.message = function(value, options) {
      options = options || {};
      angular.extend(state.options, options);
